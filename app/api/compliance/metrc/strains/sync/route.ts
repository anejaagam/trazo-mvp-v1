/**
 * Metrc Strains Sync API Route
 *
 * POST /api/compliance/metrc/strains/sync - Sync strains from Metrc to local cache
 */

import { NextRequest, NextResponse } from 'next/server'
import { createClient } from '@/lib/supabase/server'
import { canPerformAction } from '@/lib/rbac/guards'
import { syncStrainsFromMetrc } from '@/lib/compliance/metrc/sync/strains-sync'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await syncStrainsFromMetrc(client, userData.organization_id, siteId)
    ...
    console.error('Error in strains sync API:', error)
    return NextResponse.json(
