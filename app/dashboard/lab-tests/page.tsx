...
/**
 * Lab Tests Dashboard Page
 * Phase 3.5 Week 8 Implementation
 *
 * Main page for managing all lab tests and COAs
 */
...
import { COAUploadForm } from '@/components/features/lab-tests/coa-upload-form'
import { TestResultsViewer } from '@/components/features/lab-tests/test-results-viewer'
...
export default function LabTestsPage() {
  ...
  useEffect(() => {
    if (currentOrganization?.id && currentSite?.id) {
      fetchTests()
    }
  }, [currentOrganization?.id, currentSite?.id])
  ...
  const fetchTests = async () => {
    if (!currentOrganization?.id || !currentSite?.id) return
    ...
    try {
      ...
      const packagesWithTests = new Set(
        data?.flatMap(t => t.package_test_results?.map(ptr => ptr.package_id) || [])
      ).size

      const batchesWithTests = new Set(
        data?.flatMap(t => t.batch_test_results?.map(btr => btr.batch_id) || [])
      ).size
                  ...
                  <TableRow key={test.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline" className="text-xs">
                        ...
                        {test.coa_file_name.split('.').pop()?.toUpperCase()}
                      </Badge>
                    </TableCell>
                    <TableCell>
