...
export function RoomCapacityMonitor({ pods, batches }: RoomCapacityMonitorProps) {
  ...
  const podStats = pods.map(pod => {
    const podBatches = batches.filter(b => b.growingAreaIds.includes(pod.id) && b.stage !== 'closed');
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-gray-900">Room Capacity Monitoring</h2>
        <p className="text-gray-600">Track plant count and canopy area usage against licensed limits</p>
      </div>
