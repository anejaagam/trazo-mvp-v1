...
import { mockBatches, mockPods, mockGroups, mockTimelineEvents } from './lib/mock-data';
import { mockPlants, mockDryingRooms } from './lib/harvest-mock-data';
import { mockWasteLogs } from './lib/waste-mock-data';
import { mockAvailableTags } from './lib/tagging-mock-data';
import { mockCultivars, mockGenealogyRecords } from './lib/cultivar-mock-data';
...
export default function App() {
  ...
  const handleCompleteHarvest = (record: Omit<HarvestRecord, 'id' | 'harvestDate' | 'metrcReported' | 'metrcReportedAt'>) => {
    ...
    toast.success(
      `Harvest completed! ${record.plantIds.length} plants (${record.wetWeight} kg) reported to Metrc`,
      { duration: 5000 }
    );
  };
  ...
  const handleCreateCultivar = (cultivar: Omit<Cultivar, 'id' | 'createdAt'>) => {
  ...
  const handleUpdateCultivar = (id: string, data: Partial<Cultivar>) => {
    setCultivars(cultivars.map(c => c.id === id ? { ...c, ...data } : c));
    toast.success(data.isActive === false ? 'Cultivar archived' : 'Cultivar updated');
  };
  ...
  const handleCompletePackaging = (record: Omit<PackagingRecord, 'id'>) => {
    ...
    toast.success('Packaging completed and reported to Metrc', { duration: 5000 });
  };
  ...
  const handleBulkLocationUpdate = (batchIds: string[], podIds: string[], authorizedBy: string) => {
    setBatches(batches.map(b =>
      batchIds.includes(b.id) ? { ...b, podIds } : b
    ));
      ...
      <header className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-gray-900">Batch Management System</h1>
              <p className="text-gray-600">Track crop batches from seed to harvest</p>
            </div>
          </div>
        </div>
      </header>
