...
export function BulkBatchOperations({ 
  ...
  const [newGrowingAreaIds, setNewGrowingAreaIds] = useState<string[]>([]);
  ...
  const handleSubmit = () => {
    ...
    if (operationType === 'stage') {
      ...
      onBulkLocationUpdate(selectedBatchIds, newGrowingAreaIds, authorizedBy.trim());
    }

    setDialogOpen(false);
    setSelectedBatchIds([]);
    setAuthorizedBy('');
    setNewGrowingAreaIds([]);
  };

  const togglePod = (podId: string) => {
    setNewGrowingAreaIds(prev =>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-600">
                  ...
                  {' '}{batch.growingAreaIds.length} growing area(s) •
        ...
        <DialogContent className="max-w-2xl">
            ...
            {operationType === 'stage' ? (
              ...
              <div>
                  ...
                  {pods.map(pod => (
                    <div
                      ...
                      className={`p-3 border rounded-lg cursor-pointer transition-colors ${
                        newGrowingAreaIds.includes(pod.id) ? 'border-blue-600 bg-blue-50' : 'hover:border-blue-300'
                      }`}
                      onClick={() => togglePod(pod.id)}
                    >
                      <div className="flex items-center gap-3">
                        <Checkbox
                          checked={newGrowingAreaIds.includes(pod.id)}
                ...
                <p className="text-gray-500 mt-2">
                  {newGrowingAreaIds.length} growing area(s) selected
                </p>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !authorizedBy.trim() || 
                (operationType === 'location' && newGrowingAreaIds.length === 0)
              }
            >
