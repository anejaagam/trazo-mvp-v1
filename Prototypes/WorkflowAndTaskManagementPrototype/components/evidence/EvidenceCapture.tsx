...
import { EvidenceType, DualSignature } from '../../lib/types';
import { Camera, QrCode, PenTool, Upload, Check, X } from 'lucide-react';
import { DualSignatureCapture } from './DualSignatureCapture';
  ...
  switch (type) {
    ...
    case 'checkbox':
      return (
        <div className="space-y-4">
          ...
          {existingValue && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-900">Completed checklist</p>
            </div>
          )}
        </div>
      );
    ...
    case 'photo':
      return (
        <div className="space-y-4">
          ...
          <div className="border-2 border-dashed border-slate-300 rounded-lg p-8 text-center">
            {value ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  <p className="text-slate-600 mb-4">Capture or upload photo evidence</p>
          ...
          {existingValue && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-900">Photo evidence captured</p>
            </div>
          )}
        </div>
      );
    ...
    case 'signature':
      return (
        <div className="space-y-4">
          ...
          {existingValue && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-900">Signature captured</p>
            </div>
          )}
        </div>
      );
    ...
    case 'qr_scan':
      return (
        <div className="space-y-4">
          <div className="border-2 border-dashed border-slate-300 rounded-lg p-8 text-center">
            {qrScanning ? (
              ...
              <div className="space-y-4">
                ...
                <div className="p-3 bg-slate-50 rounded font-mono">{value}</div>
                <div className="flex gap-2 justify-center">
                  <Button variant="outline" onClick={() => setValue('')}>
                    <X className="w-4 h-4 mr-2" />
                    Clear
          ...
          {existingValue && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-900">QR code scanned: {existingValue}</p>
            </div>
          )}
        </div>
      );
    ...
    case 'text':
      return (
        <div className="space-y-4">
          ...
          <Button onClick={() => onCapture(value)} disabled={!value}>
            <Check className="w-4 h-4 mr-2" />
            Submit Notes
          </Button>
          {existingValue && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-900">Notes recorded</p>
            </div>
          )}
        </div>
      );
    ...
    case 'dual_signature':
      if (!config?.dualSignature) {
        return <p className="text-slate-600">Dual signature configuration missing</p>;
      }
      return (
    ...
    default:
      return <p className="text-slate-600">Unknown evidence type</p>;
  }
}
